...
You will be creating and manipulating vectors, lists, and data frames to uncover a top secret message.
...
Using the errors stating there were unexpected string constants in specific arguments. In the arguments for the vectors, there were punctuation issues that were changed to fix the errors and complete the vectors.
...
Using the error I noticed that the function cbind() was not the proper function for creating simple vectors with the other created vectors. Looking at the created vectors above, I changed the function to properly create the my_symbols function.
...
In the errors, I noticed there was punction issues with the data.frame() function, and missing quotations in the naming assignment of the created data frame.
...
**Response**:
I noticed that the function length() wasn't working so I used the internet to find the proper function for counting rows of a vector
